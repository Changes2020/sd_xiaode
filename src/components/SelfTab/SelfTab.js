...
class SelfTab extends Component {
  ...
  renderTab = (dataSource = null,
    ...
    const liList = list.map((item, index)=> {
      return (
        <span
          key={item.id}
          style={{width:`${100/(this.state.widthLength)}%`}}
  ...
  render(){
    ...
    const commonTab = !commonClass?classNames(styles.commonClass, commonClass):classNames(styles.newCommonClass, commonClass);
    const sectedTab = !tabClass?classNames(styles.tabClass, tabClass):classNames(styles.newTabClass, tabClass);
    return(
      <div>
        {this.renderTab(
          dataSource,
