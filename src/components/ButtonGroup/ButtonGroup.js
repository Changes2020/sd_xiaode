...
class ButtonGroup extends Component {
  ...
  buttonListItem(dataSource=null,dataReturnFun=null,id=null,isSelectFirst=null,spanFunction=null,btnClass=null) {
    console.log('进入到渲染button组件',dataSource,dataReturnFun,id,isSelectFirst,spanFunction,btnClass)
    const self = this;
    const data = !dataSource?{data:[]}:!dataSource.data?{data:[]}:dataSource.data
    ...
    let liList = data.map(function (item, index) {
      return (
        <Button
          className={!id?(!isSelectFirst?styles.btnStyle:(index === 0 ? styles.btnSelected : styles.btnStyle)):(item.id === id ? styles.btnSelected : styles.btnStyle)}
          key={index}
  ...
  render() {
    const {dataSource=null,dataReturnFun=null,id=null,isSelectFirst=null,spanFunction=null,btnClass=null} = this.props
    console.log('刚进入页面时候',dataSource,dataReturnFun,id,isSelectFirst,spanFunction,btnClass)
    return (
      <div>
        {this.buttonListItem(dataSource,dataReturnFun,id,isSelectFirst,spanFunction,btnClass)}
      </div>
    );
  }
}
